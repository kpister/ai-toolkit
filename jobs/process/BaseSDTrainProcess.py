...
class BaseSDTrainProcess(BaseTrainProcess):
    def __init__(self, process_id: int, job, config: OrderedDict, custom_pipeline=None):
        ...
        if self.is_caching_text_embeddings and self.trigger_word is not None:
            raise ValueError(
                "Cannot train trigger word if caching text embeddings. Please remove the trigger word or disable text embedding caching."
            )
    ...
    def run(self):
        ...
        if self.model_config.compile:
            try:
                torch.compile(
                    self.sd.unet, dynamic=True, fullgraph=True, mode="max-autotune"
                )
            except Exception as e:
