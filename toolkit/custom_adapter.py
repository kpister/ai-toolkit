...
from toolkit.models.pixtral_vision import (
    PixtralVisionEncoderCompatible,
    PixtralVisionImagePreprocessorCompatible,
)
import random
...
from toolkit.models.size_agnostic_feature_encoder import (
    SAFEImageProcessor,
    SAFEVisionModel,
)
...
class CustomAdapter(torch.nn.Module):
    def __init__(
        ...
        self.vision_encoder: Union[
            PhotoMakerCLIPEncoder, CLIPVisionModelWithProjection
        ] = None
        ...
        if self.adapter_type == "photo_maker":
            ...
            if (
                self.config.name_or_path is not None
                and self.config.name_or_path.endswith(".bin")
            ):
                self.load_state_dict(
                    torch.load(self.config.name_or_path, map_location=self.device),
                    strict=False,
                )
            ...
            if isinstance(self.sd_ref().tokenizer, list):
                ...
                self.sd_ref().tokenizer.add_tokens(
                    [self.flag_word], special_tokens=True
                )
        ...
        if self.adapter_type == "photo_maker":
            ...
            if self.config.text_encoder_arch == "t5":
                ...
                self.te = T5EncoderModel.from_pretrained(
                    self.config.text_encoder_path, torch_dtype=torch_dtype, **te_kwargs
                )
                ...
                self.te = CLIPTextModel.from_pretrained(
                    self.config.text_encoder_path
                ).to(self.sd_ref().unet.device, dtype=torch_dtype)
                self.tokenizer = CLIPTokenizer.from_pretrained(
                    self.config.text_encoder_path
                )
            else:
                raise ValueError(
                    f"unknown text encoder arch: {self.config.text_encoder_arch}"
                )
            ...
            if self.config.quantize_llm:
                ...
                self.te = AutoModel.from_pretrained(
                    self.config.text_encoder_path, **kwargs
                )
            else:
            ...
            self.vd_adapter = VisionDirectAdapter(
                self, self.sd_ref(), self.vision_encoder
            )
        elif self.adapter_type == "single_value":
            self.single_value_adapter = SingleValueAdapter(
                self, self.sd_ref(), num_values=self.config.num_tokens
            )
        elif self.adapter_type == "redux":
            vision_hidden_size = self.vision_encoder.config.hidden_size
            self.redux_adapter = ReduxImageEncoder(
                vision_hidden_size, 4096, self.device, torch_dtype
            )
    ...
    def setup_clip(self):
        ...
        if self.config.type == "photo_maker":
            ...
            if self.config.image_encoder_path is None:
                ...
                self.vision_encoder = PhotoMakerCLIPEncoder.from_pretrained(
                    self.config.image_encoder_path
                )
        elif (
            self.config.image_encoder_arch == "clip"
            or self.config.image_encoder_arch == "clip+"
        ):
            try:
                self.image_processor = CLIPImageProcessor.from_pretrained(
            ...
            self.vision_encoder = CLIPVisionModelWithProjection.from_pretrained(
                adapter_config.image_encoder_path, ignore_mismatched_sizes=True
            ...
            try:
                self.image_processor = SAFEImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except EnvironmentError:
            ...
            try:
                self.image_processor = ConvNextImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except EnvironmentError:
                print(
                    f"could not load image processor from {adapter_config.image_encoder_path}"
                )
            ...
            try:
                self.image_processor = ViTHybridImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except EnvironmentError:
                print(
            ...
            raise ValueError(
                f"unknown image encoder arch: {adapter_config.image_encoder_arch}"
            )
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        strict = False
        if (
            self.config.train_only_image_encoder
            and "vd_adapter" not in state_dict
            and "dvadapter" not in state_dict
        ):
        ...
        if "clip_fusion" in state_dict:
            self.clip_fusion_module.load_state_dict(
                state_dict["clip_fusion"], strict=strict
            )
        if "id_encoder" in state_dict and (
            self.adapter_type == "photo_maker" or self.adapter_type == "clip_fusion"
        ):
                    ...
                    for k, v in fuse_weights.items():
                        if len(v.shape) == 1:
                            ...
                            current_state_dict[k] = v[
                                : current_state_dict[k].shape[0],
                                : current_state_dict[k].shape[1],
                            ]
                        elif len(v.shape) == 3:
                            current_state_dict[k] = v[
                                : current_state_dict[k].shape[0],
        ...
        if "sv_adapter" in state_dict:
            self.single_value_adapter.load_state_dict(
                state_dict["sv_adapter"], strict=strict
            )

        if "vision_encoder" in state_dict:
            self.vision_encoder.load_state_dict(
                state_dict["vision_encoder"], strict=strict
            )
    ...
    def add_extra_values(self, extra_values: torch.Tensor, is_unconditional=False):
        if self.adapter_type == "single_value":
            if is_unconditional:
                self.unconditional_embeds = extra_values.to(
                    self.device, get_torch_dtype(self.sd_ref().dtype)
                )
            else:
                self.conditional_embeds = extra_values.to(
                    self.device, get_torch_dtype(self.sd_ref().dtype)
                )
            ...
            if self.adapter_type in ["i2v"]:
                ...
                if self.config.has_inpainting_input:
                    ...
                    if inpaint_tensor is not None and not do_dropout:
                        ...
                        inpainting_tensor_mask = F.interpolate(
                            inpainting_tensor_mask,
                            size=(
                                inpainting_latent.shape[2],
                                inpainting_latent.shape[3],
                            ),
                            mode="bilinear",
                        )
                        inpainting_tensor_mask = inpainting_tensor_mask.to(
                        ...
                        if self.config.invert_inpaint_mask_chance > 0.0:
                            do_mask_invert = (
                                random.random() < self.config.invert_inpaint_mask_chance
                            )
                        if do_mask_invert:
                            # invert the mask
                        ...
                        inpainting_latent = torch.cat(
                            (inpainting_latent, inpainting_tensor_mask), dim=1
                        )
                    else:
                        ...
                        inpainting_latent = torch.cat(
                            (
                                inpainting_latent,
                                torch.ones_like(inpainting_latent[:, :1, :, :]),
                            ),
                            dim=1,
                        )
                    ...
                    if self.config.num_control_images == 1:
                        ...
                        control_latent = inpainting_latent.to(
                            latents.device, latents.dtype
                        )
                ...
                control_tensor = batch.control_tensor.to(
                    latents.device, dtype=latents.dtype
                )
                ...
                if len(control_tensor.shape) == 4:
                    ...
                    control_tensor_list = control_tensor.chunk(
                        self.num_control_images, dim=1
                    )
                control_latent_list = []
                for control_tensor in control_tensor_list:
                    ...
                    if do_dropout:
                        ...
                        control_tensor = control_tensor.to(
                            sd.vae_device_torch, dtype=sd.torch_dtype
                        )

                        # if it is not the size of batch.tensor, (bs,ch,h,w) then we need to resize it
                        if (
                            control_tensor.shape[2] != batch.tensor.shape[2]
                            or control_tensor.shape[3] != batch.tensor.shape[3]
                        ):
                            control_tensor = F.interpolate(
                                control_tensor,
                                size=(batch.tensor.shape[2], batch.tensor.shape[3]),
                                mode="bicubic",
                            )

                        # encode it
                        control_latent = sd.encode_images(control_tensor).to(
                            latents.device, latents.dtype
                        )
                ...
                if inpainting_latent is not None:
                    ...
                    control_latent = torch.cat(
                        (inpainting_latent, control_latent), dim=1
                    )
    ...
    def condition_prompt(
        ...
        if self.adapter_type in [
            ...
            with torch.no_grad():
                ...
                if is_unconditional:
                    self.unconditional_embeds = self.te_adapter.encode_text(
                        prompt
                    ).detach()
                else:
                    self.conditional_embeds = self.te_adapter.encode_text(
                        prompt
                    ).detach()
        elif self.adapter_type == "llm_adapter":
            # todo allow for training
            with torch.no_grad():
                # encode and save the embeds
                if is_unconditional:
                    self.unconditional_embeds = self.llm_adapter.encode_text(
                        prompt
                    ).detach()
                else:
                    self.conditional_embeds = self.llm_adapter.encode_text(
                        prompt
                    ).detach()
            return prompt
            ...
            if is_unconditional:
                ...
                with torch.no_grad():
                    ...
                    for prompt in prompt_list:
                        ...
                        prompt_parts = [
                            p.strip().lower() for p in prompt_parts if len(p) > 0
                        ]
                        ...
                        if our_class is None:
                            ...
                            prompt = (
                                " ".join(
                                    [
                                        self.config.class_names[0]
                                        for _ in range(self.num_control_images)
                                    ]
                                )
                                + " "
                                + prompt
                        ...
                        boolean_mask = torch.cat(
                            (
                                boolean_mask,
                                torch.ones(self.num_control_images, dtype=torch.bool),
                            )
                        )
                        boolean_mask = boolean_mask.to(self.device)
                        # zero pad it to 77
                        boolean_mask = F.pad(
                            boolean_mask, (0, 77 - boolean_mask.shape[0]), value=False
                        )
    ...
    def condition_encoded_embeds(
        ...
        if self.adapter_type == "llm_adapter":
            ...
            if is_unconditional:
                prompt_embeds.text_embeds = (
                    self.unconditional_embeds.text_embeds.clone()
                )
                prompt_embeds.attention_mask = (
                    self.unconditional_embeds.attention_mask.clone()
                )
                return prompt_embeds
            prompt_embeds.text_embeds = self.conditional_embeds.text_embeds.clone()
            prompt_embeds.attention_mask = (
                self.conditional_embeds.attention_mask.clone()
            )
            return prompt_embeds
        ...
        if (
            self.adapter_type == "photo_maker"
            or self.adapter_type == "clip_fusion"
            or self.adapter_type == "redux"
        ):
            if is_unconditional:
            ...
            with torch.no_grad():
                ...
                if not has_been_preprocessed:
                    ...
                    if tensors_0_1.min() < -0.3 or tensors_0_1.max() > 1.3:
                        raise ValueError(
                            "image tensor values must be between 0 and 1. Got min: {}, max: {}".format(
            ...
            if self.adapter_type == "photo_maker":
                ...
                with torch.set_grad_enabled(is_training):
                    if is_training and self.config.train_image_encoder:
                        ...
                        with torch.no_grad():
                            ...
                            id_embeds = self.vision_encoder(
                                clip_image,
                                do_projection2=isinstance(
                                    self.sd_ref().text_encoder, list
                                ),
                            ).detach()

                    prompt_embeds.text_embeds = self.fuse_module(
                        prompt_embeds.text_embeds, id_embeds, self.token_mask
                    )
                    return prompt_embeds
            elif self.adapter_type == "clip_fusion":
                with torch.set_grad_enabled(is_training):
                    ...
                    prompt_embeds.text_embeds = self.clip_fusion_module(
                        prompt_embeds.text_embeds, img_embeds
                    )
                    return prompt_embeds
                ...
                with torch.set_grad_enabled(is_training):
                    ...
                    img_embeds = self.redux_adapter(
                        img_embeds.to(self.device, get_torch_dtype(self.sd_ref().dtype))
                    )

                    prompt_embeds.text_embeds = torch.cat(
                        (prompt_embeds.text_embeds, img_embeds), dim=-2
                    )
                    return prompt_embeds
        else:
            return prompt_embeds
    ...
    def get_empty_clip_image(self, batch_size: int, shape=None) -> torch.Tensor:
        with torch.no_grad():
            ...
            mean = (
                torch.tensor(self.clip_image_processor.image_mean)
                .to(self.device, dtype=get_torch_dtype(self.sd_ref().dtype))
                .detach()
            )
            std = (
                torch.tensor(self.clip_image_processor.image_std)
                .to(self.device, dtype=get_torch_dtype(self.sd_ref().dtype))
                .detach()
            )
            tensors_0_1 = torch.clip((255.0 * tensors_0_1), 0, 255).round() / 255.0
            clip_image = (tensors_0_1 - mean.view([1, 3, 1, 1])) / std.view(
                [1, 3, 1, 1]
            )
    ...
    def trigger_pre_te(
        ...
        if self.adapter_type in ["ilora", "vision_direct", "te_augmenter", "i2v"]:
            ...
            with torch.no_grad():
                ...
                # if is pixtral
                if (
                    self.config.image_encoder_arch == "pixtral"
                    and self.config.pixtral_random_image_size
                ):
                    # get the random size
                    random_size = random.randint(
                        256, self.config.pixtral_max_image_size
                    )
                ...
                if self.config.control_image_dropout > 0 and is_training:
                    ...
                    unconditional_batch = torch.chunk(
                        self.get_empty_clip_image(
                            batch_size, shape=clip_image.shape
                        ).to(clip_image.device, dtype=clip_image.dtype),
                        batch_size,
                        dim=0,
                    )
                    combine_list = []
                ...
                if (
                    self.adapter_type in ["vision_direct", "te_augmenter", "i2v"]
                    and not skip_unconditional
                ):
                    # add an unconditional so we can save it
                    unconditional = self.get_empty_clip_image(
                        batch_size, shape=clip_image.shape
                    ).to(clip_image.device, dtype=clip_image.dtype)
            ...
            if self.adapter_type == "ilora":
                with torch.set_grad_enabled(is_training):
                    ...
                    if self.config.clip_layer == "penultimate_hidden_states":
                        ...
                        raise ValueError(
                            f"unknown clip layer: {self.config.clip_layer}"
                        )
                ...
                with torch.set_grad_enabled(is_training):
                    ...
                    try:
                        if skip_unconditional:
                            self.unconditional_embeds, self.conditional_embeds = (
                                None,
                                clip_image_embeds,
                            )
                        else:
                            self.unconditional_embeds, self.conditional_embeds = (
                                clip_image_embeds.chunk(2, dim=0)
                            )
                    except ValueError:
                        raise ValueError(
                            f"could not split the clip image embeds into 2. Got shape: {clip_image_embeds.shape}"
                        )
