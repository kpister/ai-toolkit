...
class Bucket:
    def __init__(self, width: int, height: int, batch_size: int = None):
        self.width = width
        self.height = height
    ...
    def setup_buckets(self: "AiToolkitDataset", quiet=False):
        ...
        for idx, file_item in enumerate(file_list):
            ...
            if bucket_key not in self.buckets:
                # Calculate adaptive batch size for this bucket
                adaptive_batch_size = self.calculate_adaptive_batch_size(
                    file_item.crop_width, file_item.crop_height
                )
                self.buckets[bucket_key] = Bucket(
                    file_item.crop_width, file_item.crop_height, adaptive_batch_size
                )
    ...
    def get_text_embedding_info_dict(self: "FileItemDTO"):
        ...
        if self.caption is None:
            ...
            if "[trigger]" in self.caption:
                raise Exception(
                    "Error: [trigger] in caption is not supported when caching text embeddings. Please remove it from the caption."
                )
        item = OrderedDict(
            [
                ...
                if not os.path.exists(text_embedding_path):
                    ...
                    if file_item.encode_control_in_text_embeddings:
                        if file_item.control_path is None:
                            raise Exception(
                                f"Could not find a control image for {file_item.path} which is needed for this model"
                            )
                        ...
                        prompt_embeds: PromptEmbeds = self.sd.encode_prompt(
                            file_item.caption, control_images=ctrl_img
                        )
                    else:
                        prompt_embeds: PromptEmbeds = self.sd.encode_prompt(
                            file_item.caption
